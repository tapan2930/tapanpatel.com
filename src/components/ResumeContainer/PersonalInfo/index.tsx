            ...
            <div className="flex justify-around mt-4 flex-wrap">
                {
                    data.connect.map(connect=>{
                        return(
                            <div className="pb-1">{socialIcons(connect)}</div>
                        )
                    })
                }
            </div>
...
const socialIcons = (connect:any) =>{
    switch(connect.name.toLowerCase()){
        case "phone": return   <div><a className="flex items-center" href={connect.link} target="_blank"><span className="mr-3"><AiFillPhone /></span>{ connect.name }</a></div>
        ...
        case "email": return   <div><a className="flex items-center" href={connect.link} target="_blank"><span className="mr-3"><AiTwotoneMail /></span>{ connect.name }</a></div>
        case "website": return <div><a className="flex items-center" href={connect.link} target="_blank"><span className="mr-3"><RiProfileFill /></span>{ connect.name }</a></div>
        default: return null
    }
}
